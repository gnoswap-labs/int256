...
func TestSubOverflow(t *testing.T) {
	...
	for _, tc := range tests {
		...
		if got.Cmp(want) != 0 || overflow != tc.overflow {
			t.Errorf("SubOverflow(%s, %s) = (%s, %v), want (%s, %v)", 
...
func TestMulOverflow(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if gotZ.Neq(wantZ) {
			t.Errorf("MulOverflow(%s, %s) = %s, want %s", tt.x, tt.y, gotZ.ToString(), wantZ.ToString())
		}
...
func TestMulMod(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if got.Neq(want) {
			t.Errorf("MulMod(%s, %s, %s) = %s, want %s", tt.x, tt.y, tt.m, got.ToString(), want.ToString())
		}
	}
}
...
func TestExp(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if got.Neq(want) {
			t.Errorf("Exp(%s, %s) = %v, want %v", tt.x, tt.y, got.ToString(), want.ToString())
		}
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.Neq(expected) {
				t.Errorf("Test %s failed. Expected %s, got %s", tt.name, expected.ToString(), result.ToString())
			}
		})
	}
}
...
func init() {
	x = NewUint(18446744073709551615)
	y = NewUint(9223372036854775807)
	z = new(Uint)
...
func BenchmarkSubOverflow(b *testing.B) {
	for i := 0; i < b.N; i++ {
		z.SubOverflow(x, y)
	}
}
...
func BenchmarkMulOverflow(b *testing.B) {
	for i := 0; i < b.N; i++ {
		z.MulOverflow(x, y)
	}
}
